...
struct CandidateSet {
    ...
    void preprocess(const Graph& graph, const Graph& query, vector<int>& embed_order) {
        // construct embed order
        embed_order.clear();
        ...
        //     // return cs[a].size() < cs[b].size();
        //     // return query.adj[a].size() > query.adj[b].size();
        //     // return cs[a].size() * query.adj[b].size() < cs[b].size() * query.adj[a].size();
        //     // return cs[a].size() * query.adj[a].size() < cs[b].size() * query.adj[b].size();
        // });
...
int main(int argc, char** argv) {
    ...
    candidate_set.preprocess(graph, query, embed_order);
    ...
    // print degree of query
    // for (int i=0; i<query.V; ++i) {
    //     fprintf(stderr, "[%d %d %d]\n", i, query.adj[i].size(), candidate_set.cs[i].size());
    // }
    // fprintf(stderr, "\n");
