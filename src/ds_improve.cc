...
struct CandidateSet {
    ...
    // next[id][next_id]: array of next candidate index in cs array
    ...
    void preprocess(const Graph& graph, const Graph& query) {
        ...
        for (int i=0; i<cs.size(); ++i) {
            stable_sort(cs[i].begin(), cs[i].end(), [&](const CSNode& a, const CSNode& b) {
                return (multi[a.id] / 4) < (multi[b.id] / 4);
            });
        ...
        next.resize(graph.V, vector<vector<int> >(query.V, vector<int>()));
        for (int i=0; i<V; ++i) {
            for (int j=0; j<cs[i].size(); ++j) {
                int here = cs[i][j].id;
                for (int k=0; k<query.next[i].size(); ++k) {
                    int next_id = query.next[i][k];
                    for (int l=0; l<cs[k].size(); ++l) {
                        int next_v = cs[k][l].id;
                        if (graph.is_neighbor(here, next_v)) {
                            next[here][next_id].push_back(l);
                        }
                    }
                }
            }
...
struct Backtrack {
    ...
    void UpdateInfo(int size, bool add) {
        ...
        for (int i=0; i<query.next[here_id].size(); ++i) {
            ...
            int weight_sum = 0;
            for (int j=0; j<candidate_set.next[here][next_id].size(); ++j) {
                int idx = candidate_set.next[here][next_id][j];
                CSNode& next = candidate_set.cs[next_id][idx];
                // need to be improved
    ...
    bitset<500> TraverseCandidateSet() {
        if (!initiate && ++reach_cnt > (V << 15)) {
            if (matching_cnt == 0) {
        ...
        for (int i=0; i<candidate_set.cs[here_id].size(); ++i) {
            ...
            if (visited[curr.id]) {
                error_here.set(partial_embedding[embed_level[curr.id]].first, true);
                continue;
            }
            if (curr.link_cnt == query.prev[here_id].size()) {
                ...
                embed_level[curr.id] = size;
                UpdateInfo(size, true);

                bitset<500> recurse = TraverseCandidateSet();
                ...
                if (!recurse.any()) { matching_found = true; }
                else if (!recurse.test(here_id)) {
                ...
                for (int i=0; i<query.prev[here_id].size(); ++i) {
                    int prev_id = query.prev[here_id][i];
                    error_here.set(prev_id, true);
                }
            }
        }
        RemoveExtendable(size, poped);
        if (matching_found) { error_here.reset(); }
        return error_here;
    }
