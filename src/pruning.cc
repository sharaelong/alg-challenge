...
struct Backtrack {
    ...
    vector<int> extendable_rev;
    vector<int> embed_level;

    int matching_cnt = 0;
    int reach_cnt = 0;
    int ans[100] = {
        890, 2176, 893, 55, 267, 768, 39, 378, 962, 214, 2593, 752, 327, 371, 498, 495, 944, 1174, 737, 210, 573, 645, 1504, 388, 1505, 1501, 396, 1500, 1627, 90, 389, 1499, 11, 58, 1590, 1107, 816, 621, 969, 417, 1578, 167, 25, 1143, 1194, 674, 97, 27, 28, 207, 73, 304, 1029, 1712, 1713, 130, 129, 154, 1469, 1220, 796, 1468, 459, 1348, 1347, 1346, 1820, 2608, 919, 2, 1124, 1960, 252, 254, 899, 2052, 826, 2061, 823, 824, 1002, 1001, 1008, 746, 103, 744, 15, 1022, 364, 681, 982, 193, 190, 293, 1886, 14, 2318, 2845, 1971, 682
    };
    ...
    Backtrack(const string& _data, const string& _query, const string& _candidate_set) {
        ...
        extendable_rev.resize(V, -1);
        embed_level.resize(graph.V, -1);
    }
    ...
    void RemoveExtendable(int size, pair<int, int> poped) {
        ...
        for (int i=0; i<extendable_level[size].size(); ++i) {
            int next_id = extendable_level[size][i];
            extendable_rev[next_id] = -1;
        }
    ...
    int TraverseCandidateSet() {
        ...
        int result = extendable_rev[here_id];
        bool matching_found = false;
        // fprintf(stderr, "(%d %d %d)\n", size, here_id, result);
        for (int i=0; i<candidate_set.cs[here_id].size(); ++i) {
            CSNode curr = candidate_set.cs[here_id][i];
            // if (here_id == 1 && curr.id == 2176) { fprintf(stderr, "enter\n"); }
            
            // if (size == 0) {
            //     fprintf(stderr, "%d\n", curr.id);
            // }
            // fprintf(stderr, "(%d %d %d %d)\n", size, here_id, extendable_rev[here_id], curr.id);
            // if (34 <= size && size <= 36) {
            //     fprintf(stderr, "%d %d %d %d\n", curr.id, (int)visited[curr.id], curr.link_cnt, query.prev[here_id].size());
            // }
            if (visited[curr.id]) {
                result = max(result, embed_level[curr.id]);
                continue;
            }
            if (curr.link_cnt == query.prev[here_id].size()) {
                partial_embedding.push_back({ here_id, curr.id });
                visited[curr.id] = true;
                embedding[here_id] = curr.id;
                embed_level[curr.id] = size + 1;
                UpdateInfo(size, true);

                int recurse = TraverseCandidateSet();
                if (recurse == INF) { matching_found = true; }
                // if (curr.id == ans[here_id]) {
                //     fprintf(stderr, "%d %d %d\n", size, here_id, recurse);
                // }

                UpdateInfo(size, false);
                embed_level[curr.id] = -1;
                embedding[here_id] = -1;
        ...
        RemoveExtendable(size, poped);
        // fprintf(stderr, "(%d %d %d)\n", size, here_id, result);
        return matching_found ? INF : result;
    }
};
